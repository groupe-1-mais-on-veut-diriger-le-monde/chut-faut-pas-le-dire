import random
import json

import dataclasses

import copy
import Breakout

@dataclasses.dataclass
class GameState:
    distance: tuple
    position: tuple
    surroundings: str
    food: tuple


class Learner(object):
    def __init__(self, position_x, position_y, speed, width, height):

        # (x, y) : position du joueur
        self.x = position_x
        self.y = position_y
        # dx -> left/right speed
        self.dx = 0
        
        self.speed = speed
        # taille joueur
        self.width = width
        self.height = height
        self.score = 0
        self.nbr_brick_hit = 0
        # Learning parameters
        '''epsilon = 0.1 -> 10% of the time we let the bot chose an action at random'''
        self.epsilon = 1.0
        ''''''
        self.lr = 0.7
        self.discount = 0.5

        # State/Action history
        self.qvalues = self.LoadQvalues()
        self.history = []

        # Action space
        self.actions = {
            0: 'left',
            1: 'right',
            2: 'up',
            3: 'down'
        }

        # Opposite value of action so our snake doesn't illegaly eat itself by doing a 180 degree turn
        self.oposite_actions = {
            0: 1,
            1: 0,
            2: 3,
            3: 2
        }


        # Reward Parameters
        self.reward_negative = -1
        self.reward_positive_eating_food = 1
        self.reward_positive_approching_food = 1

        self.previous_key = None

    def left(self):
        self.dx = -self.speed

    def right(self):
        self.dx = self.speed

    def move(self):
        self.x = self.x + self.dx

        # Check for border collision
        if self.x < 0 + self.width / 2.0:
            self.x = 0 + self.width / 2.0
            self.dx = 0

        elif self.x > Breakout.WIDTH - self.width / 2.0:
            self.x = Breakout.WIDTH - self.width / 2.0
            self.dx = 0

    def add_points(self, brick):
        if brick.color == Breakout.YELLOW:
            self.score += 1
        elif brick.color == Breakout.GREEN:
            self.score += 3
        elif brick.color == Breakout.ORANGE:
            self.score += 5
        elif brick.color == Breakout.RED:
            self.score += 7

    def render(self):
        Breakout.pygame.draw.rect(Breakout.screen, Breakout.PLAYER_COLOR,
                     Breakout.pygame.Rect(int(self.x - self.width / 2.0), int(self.y - self.height / 2.0), self.width,
                             self.height))

    def get_rect(self):
        return Breakout.pygame.Rect(self.x, self.y, self.width, self.height)

    def Reset(self):
        self.history = []

    def LoadQvalues(self, path="qvalues.json"):
        with open(path, "r") as f:
            qvalues = json.load(f)
        return qvalues

    def SaveQvalues(self, path="qvalues.json"):
        with open(path, "w") as f:
            json.dump(self.qvalues, f)

    def get_action(self, snake, food):
        state = self._GetState(snake, food)
        print("state",state)
        # Epsilon greedy
        rand = random.uniform(0, 1)
        if rand < self.epsilon:
            keys_without_previous = list(self.actions.keys())
            if self.previous_key is not None:
                keys_without_previous.pop(self.previous_key)
            action_key = random.choices(keys_without_previous)[0]
        else:
            state_scores = self.qvalues[self._GetStateStr(state)]
            keys_without_previous = copy.deepcopy(state_scores)
            if self.previous_key is not None:
                keys_without_previous.pop(self.previous_key)

            action_key = state_scores.index(max(keys_without_previous))
        self.previous_key = self.oposite_actions[action_key]
        action_val = self.actions[action_key]

        # Remember the actions it took at each state
        self.history.append({
            'state': state,
            'action': action_key
        })
        return action_val

    def UpdateQValues(self, reason):
        history = self.history[::-1]
        for i, h in enumerate(history[:-1]):
            if reason:  # Snake Died -> Negative reward
                sN = history[0]['state']
                aN = history[0]['action']
                state_str = self._GetStateStr(sN)
                reward = -1
                self.qvalues[state_str][aN] = (1 - self.lr) * self.qvalues[state_str][
                    aN] + self.lr * reward  # Bellman equation - there is no future state since game is over
                reason = None
            else:
                s1 = h['state']  # current state
                s0 = history[i + 1]['state']  # previous state
                a0 = history[i + 1]['action']  # action taken at previous state

                '''if s1.surroundings != '0000':
                    print("current state", s1)
                    print("previous state", s0)
                    print("action taken presious state", a0)
'''
                x1 = s0.distance[0]  # x distance at current state
                y1 = s0.distance[1]  # y distance at current state

                x2 = s1.distance[0]  # x distance at previous state
                y2 = s1.distance[1]  # y distance at previous state

                if s0.food != s1.food:  # Snake ate a food, positive reward
                    reward = self.reward_positive_eating_food
                elif (abs(x1) > abs(x2) or abs(y1) > abs(y2)):  # Snake is closer to the food, positive reward
                    reward = self.reward_positive_approching_food
                else:
                    reward = self.reward_negative  # Snake is further from the food, negative reward

                state_str = self._GetStateStr(s0)
                new_state_str = self._GetStateStr(s1)
                self.qvalues[state_str][a0] = (1 - self.lr) * (self.qvalues[state_str][a0]) + self.lr * (
                            reward + self.discount * max(self.qvalues[new_state_str]))  # Bellman equation
        

    def _GetState(self, snake, food):
        # snake head is last part of the list
        snake_head = snake[-1]
        dist_x = food[0] - snake_head[0]
        dist_y = food[1] - snake_head[1]
        print("dist_x",dist_x)
        print("dist_y",dist_y)
        '''
        pos_x = 0 -> food on snake's left
        pos_x = 1 -> food on snake's right
        pos_y = 2 -> food above snake
        pos_y = 3 -> food below snake
        pos_x/pos_y = NA -> food and snake on same position
        '''
        if dist_x > 0:
            pos_x = '1'  # Food is to the right of the snake
        elif dist_x < 0:
            pos_x = '0'  # Food is to the left of the snake
        else:
            pos_x = 'NA'  # Food and snake are on the same X file

        if dist_y > 0:
            pos_y = '3'  # Food is below snake
        elif dist_y < 0:
            pos_y = '2'  # Food is above snake
        else:
            pos_y = 'NA'  # Food and snake are on the same Y file

        # list with potential future state of snake
        sqs = [
            # state snake goes left
            (snake_head[0] - self.width, snake_head[1]),
            # state snake goes right
            (snake_head[0] + self.width, snake_head[1]),
            # state snake goes down
            (snake_head[0], snake_head[1] - self.height),
            # state snake goes up
            (snake_head[0], snake_head[1] + self.height),
        ]
        #print("sqs",sqs)

        surrounding_list = []
        '''
        ex. souroundings = '0000' if no death is near
        ex. souroundings = '1000' if death is near
        '''
        for sq in sqs:
            if sq[0] < 0 or sq[1] < 0:  # off screen left or top
                surrounding_list.append('1')
            elif sq[0] >= Breakout.WIDTH or sq[1] >= Breakout.HEIGHT:  # off screen right or bottom
                surrounding_list.append('1')
            elif sq in snake[:-1]:  # part of tail
                surrounding_list.append('1')
            else:
                surrounding_list.append('0')
        surroundings = ''.join(surrounding_list)

        return GameState((dist_x, dist_y), (pos_x, pos_y), surroundings, food)

    def _GetStateStr(self, state):
        return str((state.position[0], state.position[1], state.surroundings))
